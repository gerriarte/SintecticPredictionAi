<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">{{ $t('nav.brand') }}</div>
      
      <!-- 中间步骤指示器 -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">{{ $t('process.ui.stepName') }}</div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('process.ui.graphTitle') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('process.ui.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('process.ui.relations') }}</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                <button class="action-btn" @click="refreshGraph" :disabled="graphLoading" :title="$t('process.ui.refreshTitle')">
                  <span class="icon-refresh" :class="{ 'spinning': graphLoading }">↻</span>
                </button>
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? $t('process.ui.fullscreenExit') : $t('process.ui.fullscreenEnter')">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('process.ui.graphLoading') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.ui.waitingOntology') }}</p>
            <p class="waiting-hint">{{ $t('process.ui.waitingOntologyHint') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.ui.graphBuildingTitle') }}</p>
            <p class="waiting-hint">{{ $t('process.ui.graphBuildingHint') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">{{ $t('process.ui.buildPipeline') }}</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.ui.phase1Title') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.ui.apiSpec') }}</div>
                <div class="detail-content">
                  {{ $t('process.ui.ontologyApiDesc') }}
                </div>
              </div>

              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">{{ $t('process.ui.genProgress') }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.ui.entityTypesGen', { count: projectData.ontology.entity_types?.length || 0 }) }}</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.ui.relationTypesGen', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    {{ $t('process.ui.moreRelations', { count: projectData.ontology.relation_types.length - 5 }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.ui.waitingOntologyShort') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.ui.phase2Title') }}</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.ui.apiSpec') }}</div>
                <div class="detail-content">
                  {{ $t('process.ui.graphApiDesc') }}
                </div>
              </div>

              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ $t('process.ui.waitingPrev') }}</div>
              </div>

              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">{{ $t('process.ui.buildProgress') }}</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">{{ $t('process.ui.buildResult') }}</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">{{ $t('process.ui.entityNodes') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('process.ui.relationEdges') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">{{ $t('process.ui.entityTypes') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.ui.phase3Title') }}</div>
                <div class="phase-api">{{ $t('process.ui.phase3Sub') }}</div>
              </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">{{ $t('process.ui.projectInfo') }}</span>
          </div>
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">{{ $t('process.ui.projectName') }}</span>
              <span class="item-value">{{ projectData.name }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('process.ui.projectId') }}</span>
              <span class="item-value code">{{ projectData.project_id }}</span>
            </div>
            <div class="project-item" v-if="projectData.graph_id">
              <span class="item-label">{{ $t('process.ui.graphId') }}</span>
              <span class="item-value code">{{ projectData.graph_id }}</span>
            </div>
            <div class="project-item">
              <span class="item-label">{{ $t('process.ui.simulationRequirement') }}</span>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return t('process.status.graphBuilding')
  if (currentPhase.value === 0) return t('process.status.ontologyGenerating')
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return t('process.phase.completed')
  if (currentPhase.value === phase) {
    if (phase === 1 && buildProgress.value) {
      return `${buildProgress.value.progress}%`
    }
    return t('process.phase.inProgress')
  }
  return t('process.phase.waiting')
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = t('process.errors.noPendingFiles')
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: t('process.msg.uploading') }
    ...
    if (pending.clientId) {
      formDataObj.append('client_id', pending.clientId)
    }
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.errors.ontologyFailed')
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = t('process.errors.projectInitFailed', { error: err.message || t('process.errors.unknown') })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.errors.loadProjectFailed')
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('process.errors.loadProjectFailedDetail', { error: err.message || t('process.errors.unknown') })
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || t('process.errors.processingFailed')
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: t('process.msg.startingBuild')
    }

    const response = await buildGraph({ project_id: currentProjectId.value })

    if (response.success) {
      buildProgress.value.message = t('process.msg.buildTaskStarted')
      ...
      error.value = response.error || t('process.errors.graphBuildStartFailed')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.errors.graphBuildStartFailedDetail', { error: err.message || t('process.errors.unknown') })
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || t('process.msg.processing')
      }
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: t('process.msg.buildCompleteLoading')
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.errors.graphBuildFailedDetail', { error: task.error || t('process.errors.unknown') })
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('process.ui.graphLoading'))
    return
  }
