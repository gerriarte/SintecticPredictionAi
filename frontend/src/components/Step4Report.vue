  ...
  <div class="report-panel">
    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper" ref="reportPdfSource">
          <div class="report-export-toolbar">
            ...
            >
              <span class="btn-pdf-icon" aria-hidden="true">
              ...
              {{ pdfExporting ? $t('step4.downloadPdfPreparing') : $t('step4.downloadPdf') }}
            </button>
          </div>
          <!-- Report Header -->
...
const reportPdfSource = ref(null)
const pdfExporting = ref(false)

const canExportPdf = computed(() => {
  if (!reportOutline.value) return false
  return Object.keys(generatedSections.value).length > 0
})

const handleDownloadPdf = async () => {
  const el = reportPdfSource.value
  if (!el || !canExportPdf.value) return
  pdfExporting.value = true
  await nextTick()
  try {
    const safeId = (props.reportId || 'report').replace(/[^a-zA-Z0-9-_]/g, '_')
    const date = new Date().toISOString().slice(0, 10)
    const name = `prediction-report_${safeId}_${date}.pdf`
    await exportReportElementToPdf(el, name)
    emit('add-log', t('step4.downloadPdfSuccess'))
  } catch (err) {
    console.error(err)
    emit('add-log', t('step4.downloadPdfError', { message: err?.message || String(err) }))
  } finally {
