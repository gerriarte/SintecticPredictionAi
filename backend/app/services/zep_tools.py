...
返回JSON格式的子问题列表。"""
    ...
    def interview_agents(
        ...
        """Delegate to the shared interview_runner so both ZepToolsService
        and PostgresToolsService share a single implementation.

        The flow itself never depended on Zep; it only needs the LLM client
        and the OASIS simulation runner. Helpers below (_load_agent_profiles,
        _select_agents_for_interview, _generate_interview_questions,
        _generate_interview_summary, _clean_tool_call_response) are kept for
        backward compatibility with any external caller that imported them
        directly, but are no longer used by interview_agents.
        """
    ...
    def _interview_agents_legacy_unused(
        self,
        simulation_id: str,
        interview_requirement: str,
        simulation_requirement: str = "",
        ...
        """Original ZepToolsService.interview_agents body, kept dormant for
        traceability. Removed from the call path during the cleanup that
        followed Phase 6. Do not call directly — use interview_agents()."""
